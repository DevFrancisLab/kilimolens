...
A Render Blueprint is included at the repo root (`render.yaml`).

1. Render Dashboard → **New → Blueprint** → select this repo. Render reads
   `render.yaml` and creates the `kilimolens-api` web service (root dir `backend`,
   Python 3.11).
...
3. Deploy. Render runs:
   - **Build:** `pip install -r requirements.txt` then `python -m app.ml.train`
     (training is non-fatal — the API falls back to the transparent heuristic).
